...
> **Fast path.** Most of the steps below are automated:
>
> ```bash
> node scripts/new-skill.js my-analysis --category advanced \
>   --title "My Analysis" --desc "One-line description" \
>   --zh-title "中文名" --goal "I want to …" --goal-zh "我想要…"
> ```
>
> This scaffolds `SKILL.md` with the full output contract already in place (Data Verification gate, `Data & Sources` header, Thesis Invalidation, Investment Signal block, disclaimer), generates `prompts/my-analysis.md`, adds the skill to the site's `SKILL_CATEGORIES`, inserts "goal → skill" rows in both `CHOOSE-A-SKILL` pages, adds README / cross-AI table rows and directory-tree lines, and writes a `CHANGELOG.md` line under `[Unreleased]`. You then write the analysis, run `node scripts/sync-prompts.js my-analysis`, update the two Cookbook skill counts, and run `npm test`. The manual walkthrough below explains what each of those files is for.
>
> **The prompt is generated, never hand-edited.** `prompts/<name>.md` is produced from `SKILL.md` by `scripts/sync-prompts.js` (frontmatter stripped, `/us-stock-analysis:x` → `x`, `/x` → `x`, "Claude analyzes" → "The assistant analyzes"). `npm test` runs `sync-prompts.js --check` and fails on drift. Write anything platform-specific platform-neutrally in `SKILL.md` itself.
>
> **The output contract is enforced.** `scripts/check-skill-contract.js` (in `npm test`) fails if any analysis skill lacks the Data Verification gate, the `Data & Sources` header, a Thesis Invalidation section, the signal block, or the disclaimer. Output tools, aliases, and meta skills are exempt via `scripts/lib/skill-registry.js`.
...
| Category (`--category`) | Purpose | Examples |
|--------------------------|---------|----------|
| Core Stock Analysis (`core`) | Fundamental evaluation and valuation | stock-eval, stock-valuation, technical-analysis, economics-analysis |
| Financial Reports (`reports`) | Document analysis | financial-report-analyst, 10k-digest, earnings-call-analysis |
| Market Monitoring (`monitoring`) | Activity tracking | insider-trading, dividend-analysis, short-interest |
| Advanced Research (`advanced`) | Specialized evaluation | bear-case, position-ladder, thesis-tracker, industry-map |
| Meta & Output (`meta`) | Orchestration, validation, output | full-report, result-validator, chart-master, report-generator |
| Aliases | Redirect stubs — installed, **not counted as frameworks** | fundamental-analysis → stock-eval, dcf-valuation → stock-valuation, research-bundle → full-report |

The advertised framework count is derived: skill directories − output tools − aliases (`scripts/lib/skill-registry.js`). Do not hardcode it anywhere; `npm test` checks every doc that states it.
...
├── prompts/
│   └── my-analysis.md             ← Universal prompt — GENERATED by scripts/sync-prompts.js
├── site/build/build-site.js       ← Add to SKILL_CATEGORIES (new-skill.js does this)
├── site/content/CHOOSE-A-SKILL*.md ← "goal → skill" row, both languages
├── .cursor/rules/
...
### Step 3: Generate prompts/my-analysis.md

```bash
node scripts/sync-prompts.js my-analysis
```

The universal prompt is **generated** from `SKILL.md` — never edit it by hand. `npm test` runs `sync-prompts.js --check` and fails if the two drift. The generator strips the frontmatter and rewrites Claude Code syntax (`/us-stock-analysis:x` → `x`, `/x` → `x`); anything else platform-specific must be written platform-neutrally in `SKILL.md`.

### Step 4: Manifests

Skills are **auto-discovered** from `plugins/us-stock-analysis/skills/` — there is no `skills[]` array to edit in `plugin.json`. The version is bumped at release time (`npm run release`), in all three manifests at once (`plugin.json`, `.claude-plugin/marketplace.json`, `package.json`); see the Release Timing Rule in `CLAUDE.md`. If you do bump in the branch, keep all three equal — `npm test` checks parity.

### Step 5: Update the site

Add the skill to `SKILL_CATEGORIES` in `site/build/build-site.js` (otherwise it renders under "Other"), and add a "goal → skill" row to `site/content/CHOOSE-A-SKILL.md` **and** `CHOOSE-A-SKILL-zh-TW.md`. `new-skill.js` does both. Update the skill counts in `site/content/COOKBOOK.md` and `COOKBOOK-zh-TW.md` ("N available skills") — `npm test` checks them.
...
`npm test` runs four suites: structure and counts (`test-skills.js`), the skill contract (`check-skill-contract.js`), prompt sync (`sync-prompts.js --check`), and the install script (`test-install.js`). All must pass; the exact check count grows with each skill, so no number is stated here.

To check that the skill *behaves* — not just that it is shaped right — run the opt-in behavioural eval against the fictional fixture: `EVAL_CMD='claude -p' node scripts/eval-skills.js --skills my-analysis` (see `scripts/eval-skills.js` and `data/fixtures/ZEPH.md`).
